<script lang="ts">
	...
	let { user }: { user: { name: string; email: string; avatar: string; company?: string } } = $props();
	...
	<Sidebar.MenuItem>
		<DropdownMenu.Root>
			...
			>
				...
				<DropdownMenu.Separator />
				<DropdownMenu.Group>
					<DropdownMenu.Item onSelect={() => window.location.href = '/terms'}>
						Terms
					</DropdownMenu.Item>
					<DropdownMenu.Item onSelect={() => window.location.href = '/privacy'}>
						Policy
					</DropdownMenu.Item>
				</DropdownMenu.Group>
				<DropdownMenu.Separator />
