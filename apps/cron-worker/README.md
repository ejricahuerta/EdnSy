# Lead Rosetta Cron Worker (Cloudflare)

This Worker runs on a schedule and calls the lead-rosetta app’s cron endpoints on Vercel. Use it instead of Vercel Cron to avoid cron invocation limits.
...
The same `CRON_SECRET` used by the lead-rosetta app must be set here so the app accepts the requests.
