...
/** Server logs: always-on request/event logging with [pitch-rosetta] prefix and optional timestamp */
function log(level, ...args) {
  const ts = new Date().toISOString();
  const prefix = `[pitch-rosetta] ${ts}`;
...
app.get("/api/render-test", (req, res) => {
  ...
  if (!loaded || typeof loaded.data !== "object") {
    ...
    res.status(503).set("Content-Type", "application/json").json({
      ...
      message: "Add at least one of index.json, index-dental-riverside.json, index-dental-downtown.json to the pitch-rosetta root and restart.",
    });
    return;
  }
...
app.listen(PORT, () => {
  log("info", "Pitch Rosetta API listening on", `http://localhost:${PORT}`);
