...
comment on column public.free_demo_requests.status is 'pending | creating | done | failed';

create index if not exists idx_free_demo_requests_status_created
  on public.free_demo_requests (status, created_at)
...
create index if not exists idx_free_demo_requests_email_company_status
  on public.free_demo_requests (lower(trim(email)), lower(trim(company_name)), status);
