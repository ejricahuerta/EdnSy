<script lang="ts">
	...
	function parseCsv(text: string): CsvRow[] {
		...
		for (let i = 1; i < lines.length; i++) {
			...
			const industrySlug = notionIndustryToSlug(industryRaw || 'dental');
...
<div class="page-content">
	<header style="padding-bottom: var(--space-5);">
		...
		<p class="lr-subline">
			Upload a CSV of clients. Generate demo pages, then send by email (or copy the link to send yourself). Growth and Agency users can connect their CRM instead.
		</p>
	</header>
...
{:else if atLimit}
	<div class="lr-alert lr-alert-warning mb-6">
		...
		<a href="/auth/login" class="lr-btn-primary mt-3 inline-flex">Sign in →</a>
	</div>
