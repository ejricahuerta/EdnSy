...
export const POST: RequestHandler = async (event) => {
	...
	if (!GEMINI_API_KEY) {
		return apiError(503, 'AI insights are not configured', 'MISSING_API_KEY');
	}
	...
	if (count >= CHAT_DAILY_LIMIT) {
		return apiError(
			429,
			`You've reached the daily limit of ${CHAT_DAILY_LIMIT} messages for CRM chat. Try again tomorrow.`,
			'RATE_LIMIT'
		);
	}
	...
	if (!res.ok) {
		...
		serverError('api/chat/crm', 'Gemini error', { status: res.status, body: err });
		if (res.status === 429) {
			return apiError(503, 'AI is busy. Please try again in a moment.', 'RATE_LIMIT');
		}
		return apiError(502, 'AI insights are temporarily unavailable');
	}
