/**
 * Enrich website-template index.json copy using Gemini: tagline, hero, about, and service
 * descriptions from GBP + insight (grade, summary, website grading). Keeps the same
 * structure expected by website-template (prompts/prompt.md); merges AI output into base.
 */
...
export type EnrichWebsiteTemplateCopyResult =
 ...
 */
export async function enrichWebsiteTemplateCopy(
	baseIndexJson: LandingPageIndexJson,
	prospect: Prospect,
	gbpRaw: GbpData,
	insight: GeminiInsight | null
): Promise<EnrichWebsiteTemplateCopyResult> {
	if (!GEMINI_API_KEY) {
...
For "seo": title and description must include business name and location for local search; keywords should include location, industry, and service terms.`;
	...
	try {
		...
		if (!res.ok) {
			...
			serverError('enrichWebsiteTemplateCopy', 'Gemini', { status: res.status, body: err.slice(0, 300) });
		...
		if (!parsedResult.ok) {
			serverError('enrichWebsiteTemplateCopy', 'Invalid JSON', { error: parsedResult.error, raw: text.slice(0, 200) });
			return { ok: false, error: parsedResult.error };
		}
		const parsed = parsedResult.data;

		if (!isEnrichmentShape(parsed)) {
			serverError('enrichWebsiteTemplateCopy', 'Shape mismatch', { raw: JSON.stringify(parsed).slice(0, 300) });
		...
		serverError('enrichWebsiteTemplateCopy', 'uncaught', { error: err });
