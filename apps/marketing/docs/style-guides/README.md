...
1. **Tokens**  
   The palette (`--cream`, `--ink`, `--sage`, `--amber`, etc.) is defined in **`apps/marketing/src/app.css`** under `.leadrosetta-app`. Keep those values in sync with `style-guide.html` when you change the guide.
