...
2. **Environment**
   ...
   - **Cron (optional):** A [Cloudflare Worker](cron-worker/README.md) calls `GET /api/cron/jobs/demo` every 1 minute and `GET /api/cron/jobs/gbp` every 2 minutes (avoids Vercel cron limits). Set **CRON_SECRET** (16+ chars) in both Vercel and the Worker; routes reject requests without `Authorization: Bearer <CRON_SECRET>`. Demo cron requires **SITE_ORIGIN** so demo links are correct. See [docs/v0-demo-strategy.md](docs/v0-demo-strategy.md). **Local:** Run `pnpm run cron:mock-gbp` (with dev server and `CRON_SECRET` in `.env`) to mimic the cron locally; see [scripts/README.md](scripts/README.md).
