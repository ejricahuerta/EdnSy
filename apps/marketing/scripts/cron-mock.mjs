 ...
 *   node apps/marketing/scripts/cron-mock.mjs
 *   CRON_INTERVAL_MS=60000 node apps/marketing/scripts/cron-mock.mjs   # default 60s
 *   BASE_URL=http://localhost:5173 node apps/marketing/scripts/cron-mock.mjs
 *
 * Requires in .env or .env.local (apps/marketing): CRON_SECRET
...
// Load from script's app dir (apps/marketing), then cwd so either works
loadEnvFromDir(appRoot);
...
if (!CRON_SECRET) {
	console.error('Missing CRON_SECRET. Set it in apps/marketing/.env or .env.local (e.g. CRON_SECRET=my-local-cron-secret-16chars)');
	process.exit(1);
}
