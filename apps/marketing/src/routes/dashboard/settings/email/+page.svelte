...
<div class="space-y-6">
	...
	<Card.Root class="border-0 bg-card shadow-none">
		<Card.Content class="pt-6">
			...
			<!-- Email delivery: Gmail or Resend -->
			<Card.Root class="border-0 bg-card shadow-none">
				<Card.Header>
					...
					<Card.Description>
						Sent via <a href="/dashboard/integrations" class="underline hover:no-underline">Gmail</a> when connected, or Resend as fallback.
					</Card.Description>
				</Card.Header>
				<Card.Content>
					{#if data.resendConfigured}
						<div class="flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
							<CheckCircle class="h-5 w-5 shrink-0" aria-hidden="true" />
							<span>Resend configured.</span>
						</div>
						{#if data.resendFrom}
							<p class="mt-2 text-sm text-muted-foreground">
								From: <code class="rounded bg-muted px-1.5 py-0.5">{data.resendFrom}</code>
							</p>
						{/if}
						<p class="mt-2 text-sm text-muted-foreground">
							<a href="/dashboard/integrations" class="underline hover:no-underline">Connect Gmail</a> to send from your own address.
						</p>
					{:else}
						...
						<p class="mt-2 text-sm text-muted-foreground">
							Connect <a href="/dashboard/integrations" class="underline hover:no-underline">Gmail</a> or set <code class="rounded bg-muted px-1.5 py-0.5">RESEND_API_KEY</code> in the server environment. See
							<a href="https://resend.com/docs" target="_blank" rel="noopener noreferrer" class="underline hover:no-underline">Resend docs</a>.
						</p>
					{/if}
				</Card.Content>
			</Card.Root>
