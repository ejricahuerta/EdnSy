...
import { listProspects, getProspectById, updateProspectDemoLink } from '$lib/server/prospects';
import {
	getSupabaseAdmin,
...
import { getNotionConnection } from '$lib/server/notion';
...
export const load: PageServerLoad = async ({ cookies }) => {
	...
	if (notionConnection.connected) connectedApps.push('Notion');
	const upcomingBill = await getUpcomingInvoiceForUser(user.id);
	return {
		prospects,
		prospectsPreview: prospects.slice(0, 10),
