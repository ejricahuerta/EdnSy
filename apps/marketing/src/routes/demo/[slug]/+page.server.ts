/**
 * Demo page by slug (prospect id or free_demo_requests id). When v0-generated content exists in demo-html bucket, show banner + iframe + widgets.
...
export const load: PageServerLoad = async ({ params }) => {
	...
	if (!prospect) {
		...
		if (freeDemo) {
			...
			if (freeDemo.status === 'done' && freeDemo.demo_link) {
				const demoHtml = await downloadDemoHtml(slug);
				if (demoHtml) {
	...
	if (demoHtml) {
		...
		return {
			...
			industrySlug,
			companyName,
			/** Full demo HTML for srcdoc iframe (same doc as page.html; CSS/scripts run correctly in iframe context). */
			demoHtml
		};
	}

	// Fallback: v0 Platform API demo (v0 builds and hosts; we iframe their demo URL)
	const v0DemoUrl = (scraped as { v0DemoUrl?: string } | null)?.v0DemoUrl;
	if (v0DemoUrl && typeof v0DemoUrl === 'string') {
		const gbpName = (scraped as { gbpRaw?: { name?: string } } | null)?.gbpRaw?.name;
		const companyName = prospect.companyName ?? gbpName ?? 'Demo';
		return {
			useV0Demo: true,
