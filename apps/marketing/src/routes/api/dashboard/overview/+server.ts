...
export const POST: RequestHandler = async ({ cookies }) => {
	...
	if (!res.ok) {
		...
		console.error('Gemini overview error:', res.status, err);
		if (res.status === 429) {
	...
	if (!rawText) {
		console.error('Overview: empty response from model', JSON.stringify(data).slice(0, 300));
	...
	if (!parsed) {
		console.error('Overview: could not parse overview JSON', rawText.slice(0, 300));
		return apiError(502, 'Overview generation returned invalid format. Please try again.', 'INVALID_RESPONSE');
	}
