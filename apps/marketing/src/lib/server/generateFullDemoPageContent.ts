...
- Tone and voice: Write like a credible local business, not a generic ad. Avoid tacky marketing speak, clichés, and hype (e.g. no "best in class", "we're here for you", "your trusted partner", "experience the difference"). Be factual, specific, and professional. Confident but not salesy. For trades (locksmith, plumber, HVAC, etc.) sound like a no-nonsense professional; for healthcare/salons match warmth without sounding cheesy.
...
- Stats: exactly 3 KPI items (value + label). Use business-specific numbers when available (e.g. years in business, projects completed, review count); otherwise use plausible defaults like "15+ Years", "100+ Clients", "Local & Trusted".
...
${SCHEMA}`;
	...
	try {
		...
		if (!res.ok) {
			...
			console.error('[generateFullDemoPageContent] Gemini', res.status, err.slice(0, 300));
		...
		if (!isDemoPageContentFromAiShape(parsed)) {
			console.error('[generateFullDemoPageContent] Shape mismatch', JSON.stringify(parsed).slice(0, 500));
			return { ok: false, error: 'Response shape invalid' };
		}

		return { ok: true, content: parsed };
	} catch (e) {
		console.error('[generateFullDemoPageContent]', e);
