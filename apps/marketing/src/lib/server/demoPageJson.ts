...
/** Ensure at least MIN_STATS KPI items; pad with non-duplicate defaults when missing. */
function ensureMinStats(items: Array<{ value: string; label: string }>): Array<{ value: string; label: string }> {
	if (!items?.length) return STATS_POOL.slice(0, MIN_STATS);
	if (items.length >= MIN_STATS) return items;
	const need = MIN_STATS - items.length;
