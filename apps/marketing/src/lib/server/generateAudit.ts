...
/**
 * Extract and parse JSON from Gemini response. Handles markdown code blocks,
 * trailing commas, braces inside string values (single or double quoted), and surrounding text.
 */
function parseJsonFromGeminiResponse(text: string): { ok: true; data: unknown } | { ok: false; error: string } {
	...
	// Find matching } without counting { } inside string literals
	let depth = 0;
	let end = -1;
	let i = firstBrace;
	while (i < raw.length) {
	...
	// Fallback: if brace match failed, try first { to last } or report truncation
	if (end === -1) {
		...
		// No closing brace usually means Gemini truncated the response
		if (lastBrace === -1) {
 ...
 * Kept short (3000 chars) so the audit JSON has room to complete within token limits.
 */
async function fetchWebsiteSnippet(url: string, maxChars: number = 3000): Promise<string | null> {
	try {
...
export async function generateInsightForProspect(
	...
	let websiteSnippet: string | null = null;
	if (prospect.website?.startsWith('http')) {
		websiteSnippet = await fetchWebsiteSnippet(prospect.website, 2500);
	}
...
${INSIGHT_JSON_SCHEMA}`;
	...
	const isTruncationError = (err: string) =>
		err.includes('truncated') || err === 'Response truncated (incomplete JSON); try again';

	try {
		for (const maxOutputTokens of [1024, 2048]) {
			const res = await fetch(apiUrl, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({
			...
			if (!parsed.ok) {
				const shouldRetry =
					isTruncationError(parsed.error) || finishReason === 'MAX_TOKENS';
				if (shouldRetry && maxOutputTokens === 1024) {
