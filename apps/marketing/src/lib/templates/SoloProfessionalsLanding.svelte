			...
			<div class="landing-container">
				...
				<div class="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10">
					{#each content.services.items as service}
						{@const Icon = serviceIcons[service.icon as keyof typeof serviceIcons] ?? MessageCircle}
		...
		<section class="py-20 md:py-28 bg-primary text-primary-content" aria-labelledby="cta-heading">
			<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
				...
				<a
					...
					target="_blank"
					rel="noopener noreferrer"
					class="btn btn-neutral demo-btn-lg gap-2 shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5"
				>
					{content.cta.button}
					<ArrowRight class="w-5 h-5" />
				</a>
...
<style>
.solo-professionals-landing details summary::-webkit-details-marker {
		display: none;
	}
	.solo-professionals-landing a:focus-visible,
	.solo-professionals-landing button:focus-visible {
		outline: 2px solid oklch(var(--p));
		outline-offset: 2px;
	}
	.solo-professionals-landing .section-heading {
	...
	.solo-professionals-landing .section-lead {
		color: oklch(var(--bc) / 0.8);
		line-height: 1.7;
		max-width: 42rem;
	}
	.solo-professionals-landing .prose-block {
		max-width: 42rem;
		line-height: 1.75;
	}
</style>
