<script lang="ts">
	...
	let { prospect, content = fitnessDemoContent }: { prospect: DemoProspect; content?: typeof fitnessDemoContent } = $props();
	...
	const displayName = $derived(prospect.companyName || 'Your gym');
	const heroHeadline = $derived(
		prospect.city
			? content.hero.taglineWithCity.replace('{city}', prospect.city)
			: (prospect.companyName || 'Your gym')
	);
	const heroSubline = $derived(prospect.city ? (prospect.companyName || 'Your gym') : content.hero.tagline);
	const urgencyText = content.hero.urgencyText ?? '';
	const industrySlug = 'fitness';
	const website = $derived(prospect.website);

	const serviceIcons = { Dumbbell, HeartPulse, Trophy } as const;
	const whyIcons = [Dumbbell, HeartPulse, Trophy] as const;
...
<div class="fitness-landing flex flex-col min-h-screen">
	<!-- Header -->
			...
			<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
				...
				<div class="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10">
					{#each content.services.items as service}
						{@const Icon = serviceIcons[service.icon as keyof typeof serviceIcons] ?? Dumbbell}
...
<style>
	.fitness-landing details summary::-webkit-details-marker {
		display: none;
	}
	.fitness-landing a:focus-visible,
	.fitness-landing button:focus-visible {
		outline: 2px solid oklch(var(--p));
		outline-offset: 2px;
	}
	.fitness-landing .section-heading {
	...
	.fitness-landing .section-lead {
		color: oklch(var(--bc) / 0.8);
		line-height: 1.7;
		max-width: 42rem;
	}
	.fitness-landing .prose-block {
		max-width: 42rem;
		line-height: 1.75;
	}
</style>
