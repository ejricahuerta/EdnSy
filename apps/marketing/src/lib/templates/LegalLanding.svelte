<script lang="ts">
	...
	const content = legalDemoContent;
	const displayAddress = address || content.contact.address;
	const displayName = companyName || 'Your law firm';
	...
	const serviceIcons = { Scale, FileText, ShieldCheck } as const;
	const whyIcons = [Scale, FileText, ShieldCheck] as const;
			...
			<div class="landing-container">
				...
				<div class="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10">
					{#each content.services.items as service}
						{@const Icon = serviceIcons[service.icon as keyof typeof serviceIcons] ?? Scale}
