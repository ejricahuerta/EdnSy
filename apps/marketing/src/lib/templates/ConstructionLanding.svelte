<script lang="ts">
	...
	const content = constructionDemoContent;
	const displayAddress = address || content.contact.address;
	const displayName = companyName || 'Your construction company';
	...
	const serviceIcons = { Building2, Hammer, Wrench } as const;
	const whyIcons = [ShieldCheck, Clock, Users] as const;
...
<div class="construction-landing flex flex-col min-h-screen">
	<!-- Header -->
...
<style>
	.construction-landing details summary::-webkit-details-marker {
		display: none;
	}
	...
	.construction-landing .section-heading {
	...
	.construction-landing .section-lead {
		color: oklch(var(--bc) / 0.8);
		line-height: 1.7;
		max-width: 42rem;
	}
	.construction-landing .prose-block {
		max-width: 42rem;
		line-height: 1.75;
	}
</style>
