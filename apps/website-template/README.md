...
6. **Uploads** the same HTML to Supabase Storage bucket `admin-ednsy-bucket` under `demo-html/{id}.html`.
...
From the `apps/website-template` folder. A sample **`index.json`** (Ember & Oak Bistro) is included so `npm run generate` works with default paths; replace it or use `--json=path/to/your-data.json` for your own business data.
...
Start the server, then send business data as JSON in the request body. **POST endpoints require an API key** when `DEMO_GENERATOR_API_KEY` or `WEBSITE_TEMPLATE_API_KEY` is set (e.g. on Render); send `Authorization: Bearer <key>`.
...
**Async (callback) contract:** When using `/api/generate-async`, the server returns 202 with `{ id: jobId, status: "accepted" }`. When generation finishes, it POSTs to your `callbackUrl` with header `Authorization: Bearer <callbackToken>`. Success body: `{ jobId, prospectId, userId?, publicUrl?, html? }`. When `html` is included, the callback receiver (e.g. Ed & Sy Admin app) stores it so the page loads. Failure body: `{ jobId, prospectId, userId?, error: string }`. Callback URL must be HTTPS (or HTTP for loopback: `localhost`, `127.0.0.1`). If `ALLOWED_CALLBACK_ORIGINS` is set, the URL origin must match one of the comma-separated origins (e.g. `http://localhost:5173` for local dev).
...
- **API key (optional but recommended for production):** set `DEMO_GENERATOR_API_KEY` or `WEBSITE_TEMPLATE_API_KEY` to require `Authorization: Bearer <key>` on `POST /api/generate` and `POST /api/generate-async`. If unset, those routes accept requests without auth (use only in trusted environments).
...
- **Debug logging:** set `DEBUG=1` or `DEBUG=website-template` to enable request/generation phase logs (server and CLI). Logs are prefixed with `[website-template]`.
...
**Supabase:** After writing the local file, the agent uploads the HTML to bucket `admin-ednsy-bucket` at `demo-html/{guid}.html` (a new GUID each run). Ensure that bucket exists and is allowed for your Supabase project; the script logs the storage path and public URL when upload succeeds.
