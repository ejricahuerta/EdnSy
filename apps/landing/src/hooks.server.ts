...
export const handle: Handle = async ({ event, resolve }) => {
  ...
  // Protect /demos route - only authenticated users allowed
  if (event.url.pathname.startsWith('/demos')) {
    const { session, user } = await event.locals.safeGetSession();
    
    if (!session) {
      console.log('Unauthenticated user trying to access /demos, redirecting to login');
