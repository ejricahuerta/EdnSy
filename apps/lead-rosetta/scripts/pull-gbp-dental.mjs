 ...
 * - Uses same Supabase and .env as lead-rosetta (run from apps/lead-rosetta or set env).
 ...
 *
 * Required .env (in apps/lead-rosetta): SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY,
 * GOOGLE_PLACES_API_KEY (or GOOGLE_MAPS_API_KEY), GBP_SCRIPT_USER_ID.
 *
 * Run: from apps/lead-rosetta → pnpm run pull-gbp-dental
 *      from repo root     → node apps/lead-rosetta/scripts/pull-gbp-dental.mjs (ensure .env is loaded from lead-rosetta)
 */
...
// Load .env from lead-rosetta app root (parent of scripts/)
...
async function main() {
  ...
  if (!apiKey) {
    console.error('Missing GOOGLE_PLACES_API_KEY or GOOGLE_MAPS_API_KEY. Use .env from apps/lead-rosetta.');
    process.exit(1);
  }
  if (!userId) {
    console.error('Missing GBP_SCRIPT_USER_ID (or LEAD_ROSETTA_GBP_USER_ID). Set in apps/lead-rosetta/.env.');
    process.exit(1);
  }
  if (!supabaseUrl || !supabaseKey) {
    console.error('Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY. Use .env from apps/lead-rosetta.');
    process.exit(1);
  }
