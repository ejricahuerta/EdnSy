			...
			{:else}
				<div class="overflow-x-auto mx-4 sm:mx-6 my-2">
					<Table.Root>
						<Table.Header>
							<Table.Row>
								<Table.Head class="w-[min(200px,40%)]">Business name</Table.Head>
								...
								<Table.Row>
									...
									<Table.Cell>
										{#if p.demoLink}
											<a
												...
												title={p.demoLink}
											>
												<span class="truncate">{p.demoLink}</span>
									...
									<Table.Cell class="text-right">
										<Button
											...
											onclick={() => goto(`/dashboard/prospects/${p.id}?returnTo=${encodeURIComponent('/dashboard/demos')}`)}
										>
