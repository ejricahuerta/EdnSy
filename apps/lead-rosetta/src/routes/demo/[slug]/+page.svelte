<script lang="ts">
	...
	onMount(() => {
		...
		if (useCinematicDemo && cinematicDemoSrc) {
			const clear = startIframeLoadTimeout();
			return clear;
		}
	});
</script>
...
{:else if useCinematicDemo}
	...
	<div class="demo-cinematic flex flex-col bg-stone-100">
		<div class="demo-cinematic-iframe-wrap relative min-h-[60vh]">
			{#if cinematicDemoSrc}
				<iframe
					bind:this={demoIframeRef}
					title="Demo website for {companyName}"
					src={cinematicDemoSrc}
					class="demo-cinematic-iframe w-full min-h-[60vh] border-0"
					onload={handleDemoIframeLoad}
				></iframe>
			{:else}
				<div class="absolute inset-0 z-20 flex flex-col items-center justify-center gap-4 bg-stone-100 p-6 text-center">
					<p class="text-stone-700 font-medium">{DEMO_ERROR.DEMO_PAGE_UNAVAILABLE}</p>
					<p class="text-sm text-stone-600">{DEMO_ERROR.DEMO_STILL_GENERATING}</p>
					<button
						type="button"
			...
			{#if iframeLoadTimeout}
				<div class="absolute inset-0 z-20 flex flex-col items-center justify-center gap-4 bg-stone-100 p-6 text-center">
					<p class="text-stone-700 font-medium">{DEMO_ERROR.DEMO_PAGE_LOAD_FAILED}</p>
					<p class="text-sm text-stone-600">The demo took too long to load. Try again.</p>
					<button
						type="button"
						class="rounded-md bg-stone-800 px-4 py-2 text-sm font-medium text-white hover:bg-stone-700"
						onclick={() => window.location.reload()}
			...
			{:else if iframeBlank}
				<div class="absolute inset-0 z-20 flex flex-col items-center justify-center gap-4 bg-stone-100 p-6 text-center">
					<p class="text-stone-700 font-medium">{DEMO_ERROR.DEMO_PAGE_BLANK}</p>
					<p class="text-sm text-stone-600">{DEMO_ERROR.DEMO_PAGE_PARTS_FAILED}</p>
					<button
						type="button"
						class="rounded-md bg-stone-800 px-4 py-2 text-sm font-medium text-white hover:bg-stone-700"
						onclick={() => window.location.reload()}
			...
			{:else if cinematicDemoSrc && !iframeBlank && !iframeLoadTimeout}
				<div class="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 pointer-events-none flex items-center gap-2 rounded-full bg-black/50 text-white/90 text-xs px-3 py-1.5 backdrop-blur-sm">
