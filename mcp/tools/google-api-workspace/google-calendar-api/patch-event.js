/**
 * Function to patch an event in Google Calendar.
 *
 * @param {Object} args - Arguments for the patch event.
 * @param {string} args.calendarId - The ID of the calendar containing the event.
 * @param {string} args.eventId - The ID of the event to be updated.
 * @param {Object} args.eventData - The data to update the event with.
 * @param {boolean} [args.alwaysIncludeEmail=true] - Whether to always include the email address of attendees.
 ...
 * @param {string} [args.sendUpdates="all"] - Indicates who should receive updates.
 * @param {boolean} [args.supportsAttachments=true] - Whether the event supports attachments.
 * @returns {Promise<Object>} - The result of the patch event operation.
 */
const executeFunction = async ({ calendarId, eventId, eventData, alwaysIncludeEmail = true, conferenceDataVersion = 54806309, maxAttendees = 54806309, sendNotifications = true, sendUpdates = 'all', supportsAttachments = true }) => {
  ...
  try {
    ...
    return { error: 'An error occurred while patching the event.' };
  }
};

/**
 * Tool configuration for patching an event in Google Calendar.
 * @type {Object}
 */
const apiTool = {
  ...
  definition: {
    ...
    function: {
      name: 'patch_event',
      description: 'Patch an event in Google Calendar.',
      parameters: {
        type: 'object',
        properties: {
          calendarId: {
          ...
          alwaysIncludeEmail: {
            type: 'boolean',
            description: 'Whether to always include the email address of attendees.'
          },
          ...
          sendUpdates: {
            type: 'string',
            description: 'Indicates who should receive updates.'
          },
          supportsAttachments: {
            type: 'boolean',
